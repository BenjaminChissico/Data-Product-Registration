...
def data_product_form(blob_handler: bs.BlobStorage) -> dp.DataProductDetails:
    ...
    with st.form("metadata-registration-form"):
        ...
        <p align=middle>Please provide all necessary information to catalogue your new Data Product and make it accessable.</p><br><br>""",
        ...
        with col1:
            st.markdown(
                "<h5 align=middle>Mandatory Information</h5>", unsafe_allow_html=True
            )
        ...
        with col2:
            st.markdown(
                "<h5 align=middle>Optional Information</h5>", unsafe_allow_html=True
            )
        ...
        if not is_sub and (
            not dp_name or not description or not domain or not data_owner
        ):
            st.stop()
    ...
    if blob_handler.data_product_exists(dp_name.strip()):
        st.error(
            f"The Data Product: {dp_name} already exists, please provide another name"
        )
        is_sub = False
        st.stop()
    ...
    dp_details_dct = {
        ...
        "tags": tags,
        "access_details": access_details_obj,
    }
...
def register_product(
    file: UploadedFile, data_product_details: dp.DataProductDetails, blob_handler
) -> dp.DataProductDetails:
    ...
    for bytes_data, file_name in zip_file.extract_dp_items():
        ...
        table = create_table_n_column_details(
            file_name, data_product_details, bytes_data, dr.DataReader
        )
        tables.append(table)
...
def create_table_n_column_details(
    file_name: str,
    data_product_details: dp.DataProductDetails,
    data: bytes,
    data_reader: dr.DataReader,
) -> dp.DataProductDetailSampleDataTable:
    ...
    table = dp.DataProductDetailSampleDataTable(
        file_name,
        data_product_details.schema_version,
